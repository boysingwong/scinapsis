...
try:
    ...
    query = ("SELECT id, src_address FROM scin_db.scin_pub_meta WHERE id between 90001 and 100000 ORDER BY id")
    mysql_cursor.execute(query)

    for (id, src_address) in mysql_cursor:
		print "PROCESS URL = %s, id = %d " % (src_address, id)
		command = 'scrapy.exe crawl PlosonePatch -a start_url="%s" -a doc_id="%s"  -s LOG_FILE=patch_output.txt ' % (src_address, id)
		os.system(command)
