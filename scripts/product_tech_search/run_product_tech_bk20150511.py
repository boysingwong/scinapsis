...
def search_product_tech(doc_id):
	...
	try:
		...
		query = ("SELECT figure_id, si_id, tech_id, tech_parental_name, tech_alternative, prod_id, supplier, catalog_nb, prod_name_id, product_name, content FROM scin_db.pub_tech_prod_temp")
		mysql_cursor.execute(query)
		
		rsltCount = 0
		for (figure_id, si_id, tech_id, tech_parental_name, tech_alternative, prod_id, supplier, catalog_nb, prod_name_id, product_name, content) in mysql_cursor:
		  outputStr = "result: %s, %s, %s, %d " % (tech_parental_name, tech_alternative, product_name, figure_id, si_id)
		  ...
		  for sentence in sentenceList:
			...
			if technique_exists and protein_exists:
				insertStmt = ("INSERT INTO scin_db.pub_tech_prod_result "
							  "(doc_id, figure_id, si_id, tech_id, tech_parental_name, tech_alternative, prod_id, supplier, catalog_nb, product_name, sentence) "
							  "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)")
				mysql_cursor.execute(insertStmt, (doc_id, figure_id, si_id, tech_id, tech_parental_name, tech_alternative, prod_id, supplier, catalog_nb, prod_name_id, product_name, sentence) )
				mysql.commit()
